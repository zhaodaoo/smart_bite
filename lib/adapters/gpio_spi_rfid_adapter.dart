...
class GPIOSPIRFIDReaderManager extends ChangeNotifier
  ...
  Future<List<RFIDReading>> scanAll() async {
    ...
    try {
      ...
      final readerResults = await compute(_performScanInIsolate, configsData);

      final cardCount = readerResults.values.where((tag) => tag != null).length;
      debugPrint('Scan complete. Found $cardCount cards: $readerResults');
      ...
      for (final config in _configs) {
        final tagId = readerResults[config.deviceId];

        final reading = (tagId != null && tagId.isNotEmpty)
            ? RFIDReading.success(
                config.deviceId,
                tagId, // Already in hex format from SimpleMFRC522
              )
            : RFIDReading(
                deviceId: config.deviceId,
  ...
  static Future<Map<String, String?>> _performScanInIsolate(
      List<Map<String, int>> configsData) async {
    ...
    try {
      ...
      return await pollingService.performOneLoopCycles(readerConfigs).timeout(
